...
public class OrderDaoImpl implements OrderDao {
    ...
    public ArrayList addCart(int orderId) {
        ...
        try (PreparedStatement ps = MySqlConnection.createConnection().prepareStatement("select * from addcart_orders where order_id=? and order_status=?");
                PreparedStatement ps1 = MySqlConnection.createConnection().prepareStatement("select * from addcart_lineitems where order_id=?");) {
            ps.setInt(1, orderId);

...
public class Statistics {
  ...
  public List<Customerr> getPaidCustomer() throws SQLException {

    List<Customerr> ls = new ArrayList<>();
    try (PreparedStatement pg = MySqlConnection.createConnection().prepareStatement("select * from Customer where cus_bal  in (0)")) {
      ...
      while (rs.next()) {
        ls.add(new Customerr(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getDouble(6)));
      }
    }
 ...
 return ls;
  ...
  public List<Customerr> getUnPaidCustomer() throws SQLException {

    List<Customerr> ls1 = new ArrayList<>();
    try (PreparedStatement pg1 = MySqlConnection.createConnection().prepareStatement("select * from Customer where cus_bal  not in (0)")) {
      ...
      while (rs.next()) {
        ls1.add(new Customerr(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getDouble(6)));
      }
  ...
  public List<PaymentDetails> getPaymentDetails() throws SQLException {

    List<PaymentDetails> ls1 = new ArrayList<>();
    try (PreparedStatement pg2 = MySqlConnection.createConnection().prepareStatement("select * from Customer,payment where Customer.customer_id=payment.customer_id")) {
  ...
  public List<OrderedDetails> getOrdersByDate(Date date) throws SQLException {
    List<OrderedDetails> ls1 = new ArrayList<>();
    try (PreparedStatement pg3 = MySqlConnection.createConnection().prepareStatement("select * from Customer join Orders on Customer.customer_id=Orders.customer_id where OrderedDate=?");) {
      pg3.setDate(1, date);

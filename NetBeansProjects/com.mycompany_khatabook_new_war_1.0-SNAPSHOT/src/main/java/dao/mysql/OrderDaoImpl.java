...
public class OrderDaoImpl implements OrderDao {
    ...
    public boolean addOrder(Orderrs o, LineItemss l[]) {
        int order = 0, line = 0;
        try (PreparedStatement ps = KhataBookDaoImpl.getDataSource().getConnection().prepareStatement("insert into Orders values(?,?,default,?)"); PreparedStatement ps1 = MySqlConnection.createConnection().prepareStatement("insert into line_item values(?,?,?,?,?)");) {
    ...
    public void show() {
        //  boolean sel_a = true;
        try {
            PreparedStatement ps_j = KhataBookDaoImpl.getDataSource().getConnection().prepareStatement("select Customer.customer_id,Customer.customer_name,Customer.customer_phoneno,Customer.customer_aadhar,Customer.cus_address,Orders.total_amount ,Orders.order_id,Orders.OrderedDate from Customer,Orders where Customer.customer_id=Orders.customer_id");
    ...
    public boolean addCartItems(AddCart o, LineItemss l[]) {
        int order = 0, line = 0;
        try (PreparedStatement ps = KhataBookDaoImpl.getDataSource().getConnection().prepareStatement("insert into addcart_orders values(?,?,default,?,?)"); PreparedStatement ps1 = MySqlConnection.createConnection().prepareStatement("insert into addcart_lineitems values(?,?,?,?,?)");) {
            ps.setInt(1, o.getOrderId());
